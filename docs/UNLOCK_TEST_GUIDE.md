...
4. Để trống ô URL (hệ thống dùng `CLICK_UNLOCK_URL`) rồi **Lưu thay đổi**. Dòng "Đang áp dụng" chuyển sang chế độ link, revision tăng 1.
   - **Link tùy chọn (M4):** tắt công tắc, nhập URL HTTPS công khai bất kỳ (ví dụ `https://www.wikipedia.org/`), **Lưu**. Tải lại trang, URL vẫn còn. Bật công tắc rồi **Lưu**: "Đang áp dụng" hiện link. Nếu server chưa có `CLICK_UNLOCK_ENABLED=true` hoặc secret, lưu khi bật công tắc sẽ báo đúng điều kiện còn thiếu (409), bản nháp vẫn giữ nguyên, URL đã lưu trước đó không mất.
...
| 3 | Bấm nút | URL đã cấu hình (`example.com` hoặc link tùy chọn) mở trong **tab mới**; trang đọc không tự chuyển hướng hay bật popup. |
...
| 8 | Trong khi còn hạn, admin tắt mở khóa, đổi phương thức hoặc **đổi URL** rồi lưu | Chương khóa bị khóa lại ngay ở request kế tiếp (revision mới làm grant cũ mất hiệu lực). |
| 9 | Nhập URL `https://shopee.vn/...`, `http://...`, `https://127.0.0.1/`, `https://localhost/`, `https://nas.local/` rồi lưu | Bị từ chối ngay trên form và ở server, kèm lý do (Shopee cần chấp thuận riêng; chỉ https; phải là tên miền công khai). URL đã lưu trước đó giữ nguyên. |
...
Chạy trên cùng database test, với app local đang chạy ở cổng 3100 và `CLICK_UNLOCK_URL=https://example.com/`. Script **chỉ** nhận URL localhost và `DATABASE_URL` có tên `storyweb_test`, `storyweb_m3_*` hoặc `storyweb_m4_*` vì nó thay đổi cài đặt admin và tạo/xóa truyện thử:
...
- URL `http:`/`javascript:`/`data:`/`file:`, có thông tin đăng nhập, IP (kể cả dạng mã hóa, IPv6), localhost, tên miền nội bộ, sai cú pháp, quá 2048 ký tự và Shopee (khi chưa có chấp thuận) đều bị từ chối;
- (M4) URL công khai khác Shopee (`https://www.wikipedia.org/...`) lưu được khi công tắc tắt, lưu ở dạng chuẩn, tải lại vẫn còn, rồi bật được; `/unlock/visit` redirect tới đúng URL đó; đổi URL làm tăng revision và thu hồi grant; `example.com` vẫn chạy; `shopee.vn.evil-example.com` không bị coi là Shopee;
- (M4, tùy chọn `STORYWEB_TEST_URL_FLAG_OFF`) trên instance `CLICK_UNLOCK_ENABLED=false` cùng DB: URL vẫn lưu được khi tắt công tắc; bật công tắc trả `409 mode_not_ready` với `fields.unlockMode` (không phải `fields.linkUrl`) và URL đã lưu không mất;

...
| --- | --- | --- | --- | --- | --- |
| C1 | Claude Code | Dọn codebase: export/hằng không dùng, CSS chết, test script, docs cũ, rác local | Đang làm (Claude Code), nhánh `claude/cleanup`; người dùng duyệt plan, giữ code migrate localStorage | `src/lib/*` (export thừa), `ad-slot.tsx` (kiểu trùng), `unlock.ts` + 2 route (cookie cũ), `globals.css`/`dark-overrides.css` (chỉ xóa rule chết, người dùng cho phép), `package.json` scripts, `scripts/import-stories.mjs`, `docs/` (archive), `CLAUDE.md`, workboard | Không đổi hành vi, quyền đọc, unlock, schema/migration. Typecheck/lint/build, unit test, smoke. |

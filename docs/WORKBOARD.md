...
| B1 | Claude Code của người dùng | Xác thực nhiều tác giả và tài khoản độc giả | Hoàn tất, chờ Codex review/tích hợp | Nhánh `claude/b1-multi-user-auth`. Đã sửa: `src/db/schema.ts`, `drizzle/0002_b1_accounts_sessions.sql` + `drizzle/meta`, `src/lib/{auth,password,rate-limit,request-guard,cms-access}.ts` (mới), `src/lib/managed-stories.ts`, `src/app/api/auth/{register,login,logout,session,account}` (mới), `src/app/api/admin/users/**`, `src/app/api/admin/stories/owner` (mới), `src/app/api/admin/{stories,login,logout}/route.ts`, `src/app/panel/page.tsx`, `scripts/{create-user,test-auth,test-migration}.mjs` (mới), `docs/AUTH_API.md` (mới). Không đổi `package.json`/lockfile, CSS, components | Contract API, rollback và cách tạo editor: `docs/AUTH_API.md`. Session DB (cookie `storyweb_session`, 30 ngày, role đọc lại từ DB), scrypt, `stories.owner_id` nullable (truyện cũ = admin), rate limit trên PostgreSQL, kiểm tra `Origin`. Admin mật khẩu cũ giữ nguyên (thêm 429). Không cần env mới. `typecheck`/`lint`/`build` đạt. Test 2026-09-23 trên database riêng `storyweb_b1_test`/`storyweb_b1_migration_test` (server Postgres Railway, không đụng DB `railway` production; đã xóa sau test): `test-migration.mjs` đạt (0000+0001+dữ liệu → 0002 không mất dòng), `db:migrate` đạt, `test-auth.mjs` đạt 14/14 nhóm qua 2 instance (cổng 3100/3101), `smoke.mjs` (gate tắt) đạt. Production chưa migrate `0002`. Còn thiếu: UI đăng nhập/đăng ký/quản lý tài khoản (Antigravity); Codex xác nhận Railway thêm IP vào cuối `X-Forwarded-For`; tắt TCP Proxy Postgres nếu không cần |

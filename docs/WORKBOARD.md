...
| B1 | Claude Code của người dùng | Xác thực nhiều tác giả và tài khoản độc giả | Đang làm (Claude Code) | Worktree `.worktrees/claude-b1`, nhánh `claude/b1-multi-user-auth`. Dự kiến sửa: `src/db/schema.ts`, `drizzle/0002_*` + `drizzle/meta`, `src/lib/{auth,password,rate-limit,request-guard,cms-access}.ts` (mới), `src/app/api/auth/**` (mới), `src/app/api/admin/users/**` (mới), `src/app/api/admin/stories/**`, `src/lib/managed-stories.ts`, `src/app/panel/page.tsx`, `src/app/api/admin/{login,logout}/route.ts`, `scripts/create-user.mjs`, `scripts/test-auth.mjs` (mới), `docs/AUTH_API.md` (mới). Không sửa `package.json`/lockfile | Session theo tài khoản, quyền editor/admin, rate limit; Codex review trước tích hợp |
